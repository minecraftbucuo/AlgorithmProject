...
// 78. https://www.luogu.com.cn/problem/CF576A
// #include <bits/stdc++.h>
// #define int long long
...
// 79. https://www.luogu.com.cn/problem/CF888G
// #include <bits/stdc++.h>
// #define int long long
//
// struct node {
...
//     int u = 0;
//     tr[u].l = std::min(tr[u].l, id);
//     tr[u].r = std::max(tr[u].r, id);
//     for (int i = 29; i >= 0; i--) {
//         int b = x >> i & 1;
//         if (!tr[u].ch[b]) {
//             tr[u].ch[b] = tr.size();
...
//     tr.emplace_back();
//     for (int i = 1; i < a.size(); i++) insert(a[i], i);
//     int ans = 0;
//     auto get_ans = [&](auto&& self, int bit, int u) -> void {
//         int lc = tr[u].ch[0], rc = tr[u].ch[1];
//         if (lc && rc) {
//             int mi = 1e9;
//             int cl = tr[lc].r - tr[lc].l + 1;
//             int cr = tr[rc].r - tr[rc].l + 1;
//             if (cl > cr) std::swap(lc, rc);
//             for (int i = tr[lc].l; i <= tr[lc].r; i++) {
//                 int num = a[i];
//                 int sum = 1 << bit;
//                 int t = rc;
...
//             ans += mi;
//         }
//         if (lc) self(self, bit - 1, lc);
//         if (rc) self(self, bit - 1, rc);
//     };
//     get_ans(get_ans, 29, 0);
//     std::cout << ans << "\n";
//     return 0;
// }
